...
@register.inclusion_tag('weather/embed.html')
def get_weather():
    wx = Current.objects.get(pk=1)
    weather = wx.getInfo(wx)
    return {'weather': weather}

@register.inclusion_tag('weather/quicklook.html')
def quicklook(data,unitSpeed,daynum):
    cTime = time.strftime('%H', time.localtime(time.time()))
    if cTime > 14:
        block1 = {
            'header': 'Tonight',
            'temp': data['forecast'][0]['low'],
            'sunset': data['forecast'][0]['sunset'],
            'icon': data['forecast'][0]['night']['icon'],
            'type': data['forecast'][0]['night']['type']
        }
        block2 = {
            'header': data['forecast'][1]['Day'],
            'temp': data['forecast'][1]['high'],
            'sunset': data['forecast'][0]['sunset'],
            'sunrise': data['forecast'][1]['sunrise'],
            'icon': data['forecast'][1]['day']['icon'],
            'type': data['forecast'][1]['day']['type']
        }
    else:
        block1 = {
            'header': 'Today',
            'temp': data['forecast'][0]['high'],
            'sunset': data['forecast'][0]['sunrise'],
            'icon': datadata['forecast'][0]['day']['icon'],
            'type': data['forecast'][0]['day']['type']
        }
        block2 = {
            'header': 'Tonight',
            'temp': data['forecast'][0]['low'],
...
@register.inclusion_tag('weather/details.html')
